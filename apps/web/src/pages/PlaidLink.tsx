...
// One-time setup page (not in the nav — visit /plaid-link directly): runs the
// Plaid Link flow against the bank, then shows the permanent access token to
// paste into .env as PLAID_ACCESS_TOKEN.
      ...
      <section className="mt-3 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
        {!accessToken ? (
          <>
            ...
            >
              {busy ? "Working…" : "Connect bank via Plaid"}
            </button>
            {status && <p className="mt-3 text-sm text-zinc-400">{status}</p>}
          </>
        ) : (
          <>
            <p className="text-sm text-emerald-400">Bank connected. Final step:</p>
            <ol className="mt-2 list-inside list-decimal space-y-1 text-sm text-zinc-300">
              <li>
                Copy this access token into <code>.env</code> as{" "}
                <code>PLAID_ACCESS_TOKEN=…</code>
              </li>
              <li>Restart the API — transactions sync on boot, then every 5 minutes.</li>
            </ol>
            <pre className="mt-3 overflow-x-auto rounded-lg border border-zinc-700 bg-zinc-950 p-3 text-xs text-zinc-100">
              {accessToken}
            </pre>
            <p className="mt-2 text-xs text-zinc-500">
              Treat this like a password — it grants read access to your bank transactions.
            </p>
          </>
        )}
      </section>
