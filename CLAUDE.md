...
- **DB-mutating buttons get `cursor-pointer`** (+ `disabled:cursor-not-allowed` where they have a disabled state). Tailwind v4's preflight dropped the default `cursor: pointer` on `<button>`, so any control that writes to the DB — completing/clearing/deleting todos, saving a day-log, add/edit exercise or book, toggling a period day, uploading holdings — must set it explicitly. Shared controls already do it in one place (`CompleteButton` in `lib/todos.tsx`, the save button in `components/DayLog.tsx`), so those propagate everywhere. Non-mutating buttons (nav, expand/collapse, Cancel) intentionally keep the default cursor.
