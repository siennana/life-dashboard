...
- **Value history**: `buildPortfolio` upserts a daily `metrics` row (`source: "fidelity"`, `name: "portfolio_value"`, last write of the day wins) on every priced build — page loads *and* the 5-min sync loop — so the series accumulates without visits (live snapshots since 2026-08-07, PC only since it needs `FINNHUB_API_KEY`). Served as `history` in the portfolio response; the chart renders once ≥2 days exist.
...
- **Expanded-day detail** (`DayForm` → left column: `DayLog`, `TodosForDay`, `TransactionsForDay`; right column: `DaySchedule`). The log (`DayLog.tsx`) has **no save button — it autosaves on blur** (only when dirty); text is greyed (`text-zinc-400`) while locked and turns white on focus (`focus:text-zinc-100`) to signal edit mode. Todos (due + completed, from the shared `["todos"]` query) and Transactions (`GET /api/finance/transactions/:date` → `buildDayTransactions`, all plaid rows on that day, money-in green with `+`) are read-only lists.
